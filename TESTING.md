...
- [ ] *Send Files to Distro...* copies to `~`; to `/root` it reports no permission and changes nothing; sending an existing name asks, in the prompt, to overwrite or skip; sending a backup asks, in the prompt, *Send and extract* or *Only send*, and extracting restores it. No question appears as a notification.
